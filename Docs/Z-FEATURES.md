...
`nuvio-z`, 176 in `NuvioZDesktop`, 12 in `NuvioZWeb`. Last updated 2026-09-17 — revision 8, which
set the Android, iOS and TV **targets** for Phase 6 and Phase 9. Desktop has since shipped
`0.1.23-alpha-z2` and hotfixed through `z6`, so `**branch**` rows in §0 are now released on desktop
and want a state sweep; that sweep is Phase 6 Stage A work and is deliberately not done here, to
avoid guessing at it.
...
| **branch** | built and unreleased when the row was written. ⚠ **Stale as of 2026-09-17:** this pointed at `claude/setup-wizard-final-pass-wy7csp` and called it `0.5.0-beta` material; desktop's canonical line is now `nuviozdesktop` `Dev`, and much of what is marked `**branch**` has shipped in `0.1.23-alpha-z2`..`z6`. Re-sweep the state column in Phase 6 Stage A |
