...
| **W4** | **Show-once-by-revision** - an integer, profile-scoped and synced, currently at **revision 7** on desktop and 6 on mobile. A boolean could never re-ask, and keying on the app version would re-show the whole wizard every release. Finishing also marks the old mode-selector seen, so a downgrade does not re-prompt. Settings, About, **Run setup again** re-runs it dismissibly, over the app rather than gating it. | **branch** | ships | yes | yes | yes | **no** |
...
| **The setup wizard** | Revision 6 walked end to end on a real desktop build (maintainer, 2026-09-10). | ⚠ Revision 7 has **not** been on hardware at all, and `Settings → Run setup again` has had no physical result since the pointer-consumption fix. |
