...
| **P17** | **Streamlined quality table (desktop)** - the wide branch replaces the scaled-up phone card grid with a quality table: one row per option, grouped by resolution, with Needs, Size and connection fit as baseline-aligned columns. The phone branch is unchanged and serves handsets and small tablets. | **branch** | ships | n/a | n/a | yes | no |
