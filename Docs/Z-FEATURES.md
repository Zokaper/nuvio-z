...
Base: NuvioMobile `979d5680` (2026-07-29). 245 commits in `nuvio-z`, 176 in `NuvioZDesktop`, 12 in
`NuvioZWeb`. Last updated 2026-08-23.
...
| **N3** | **The figure does not move while it is read** - the sheet used to show a stale number, then "Checking", then a new one. "Checking" is now tested first, on the same pure predicate the probe obeys, and the null travels down to the cards so meters and warnings cannot jump separately. | **branch** | yes | yes | yes | **port**, Phase C |
...
| **C5** | **Fork update line** - in-app updates repointed from upstream to `Zokaper/nuvio-z`, which is why that repo must stay public: the updater is unauthenticated. The release-channel filter was cleared - it matched a branch name against a release's target commitish, which is a commit SHA, so it rejected every release. | shipped | yes | yes | yes | **verify** |
