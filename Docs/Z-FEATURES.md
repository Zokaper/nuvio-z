...
`nuvio-z`, 176 in `NuvioZDesktop`, 12 in `NuvioZWeb`. Last updated 2026-09-21 — **revision 11**.
...
Phase 9's TV target was decided early. They exist so a *planned* state can be recorded precisely
...
purpose")*. Phase 0 left it unset so that Phase 9 could cherry-pick the whole list, but that left
the ledger unable to answer what the television is *for*, which is a product question and not an
implementation detail. So the target is recorded here now and Phase 9 still owns building it, the
...
> porting notes; Phase 9 settles the remaining per-feature judgement calls against this target
...
**Phase 9 web drift:** NuvioZWeb reimplements the Kotlin playback decisions rather than sharing
...
| **The web port on a television** | It ran on a TV. | Buggy. Phase 9 owns the bugs; the claim being settled here is only that it runs at all. |
