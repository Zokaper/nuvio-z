...
> ⚠ **Five rows are missing a platform cell** and need their author's intent: **P11**, **P12**,
> **C17**, **C18**, **C19** each carry six columns where the table has seven, so at least one
> platform value is either absent or shifted. P11 reads `AND=n/a iOS=yes DSK=n/a` for a feature
> whose own text says *desktop only*, which cannot be right. Do not read these five rows'
> platform columns until they are corrected.
