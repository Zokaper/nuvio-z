...
**Revision 13 refines C20 into a hand-holding setup assistant.** Every step now names its purpose,
actions, success state, exact confirmation and contextual recovery help, within a restrained
five-phase layout. The terminal scripts remain advanced fallbacks.
...
| **C20** | **Portable iOS setup wizard** - a dedicated Compose Desktop utility guides SideStore setup one human task at a time, independently verifies computer/USB/iloader prerequisites, persists only non-sensitive progress, makes pairing and first refresh explicit gates, keeps stable as the normal channel, and packages self-contained Windows/macOS app images. Every page explains why the step matters, gives exact actions and an observable success state, and offers contextual troubleshooting. Human-controlled gates use a named, prominent completion action rather than a checkbox; the progress rail groups the 14 tasks into five calm phases. The final handoff separately explains routine re-signing and app updates, then persists a completed screen until the user deliberately returns to the start. Closing iloader is never a completion signal. The old shell scripts remain advanced fallback tools. | **branch** | ships | n/a | n/a | yes | n/a |
