...
**Revision 13 is the Phase 8 closeout (2026-09-24).** D16 moves to `.48`: the submitted-window
background transfers are physically verified on an iPhone (downloads continue while locked, `.46`
and `.47`), and `.48`'s fixes for a resumed 206 and finished-task re-attachment are CI-verified only.
The TV phase is renumbered **Phase 9 → Phase 10** throughout, because Downloads Redesign is now Phase
9. **No stable mobile release follows Phase 8.** Mobile stable waits for Phase 9's gate
(`ROADMAP.md`). Nothing in the Social and Watch Together iOS cells moves: they stay `**defer**` until an iOS hardware pass,
which is on the mobile stable release gate together with the `NowPlayingController` transport
bypass and iOS `engineReadiness`.

**Revision 12 adds C20, the portable Compose Desktop iOS setup wizard.** It replaces the terminal
bootstrap as the intended onboarding path while retaining both scripts as advanced fallbacks.
...
`nuvio-z`, 176 in `NuvioZDesktop`, 12 in `NuvioZWeb`. Last updated 2026-09-24 — **revision 13**.
Revision 8 set the Android, iOS and TV **targets** for Phase 6 and the TV phase (Phase 9 then, Phase 10 now) and left the state column
stale, because desktop had since shipped `0.1.23-alpha-z2` and hotfixed through `z6` while 57 rows
still read `**branch**`.

**Revision 11 is the Phase 6 closeout (2026-09-21).** S1-S8 move to `AND yes`: the Social tab, presence and Watch Together have run on the S25 against desktop in both directions, Away and the phone layouts included. Their iOS cells move from `port` to `**defer**`, not to `yes`: the same shared implementation is present and the iOS build is green in CI, but §12 forbids an iOS Social/Watch Together `yes` on the strength of a compile, and two iOS adapter items are open (see the closeout note in §0). S14-S16, W8 and W9 read `port` although the code had arrived with the convergence merge; a drift check against desktop finds those files identical, so they are `yes`. W10 is `AND yes`; its iOS text claimed an adaptation that does not exist and is corrected.
...
everything merged before it shipped. Those 54 rows now read `shipped **DSK**`, a new state that says
released-on-desktop-only rather than overloading `shipped`, which the legend defines against the
mobile line. Three rows still read `**branch**` — **D11** and **C9** (mobile) and **C19** (web) —
because desktop's release line never carried them. Shipping is not verification: rows whose text says *unverified on hardware* keep
saying it.

The sweep also turned up **three** rows carrying the exact defect the Phase 2 audit fixed for
**P11** — a missing cell shifting every value one column left. **C17** and **C18**, whose own text
says *(desktop only)*, read `AND=n/a iOS=yes DSK=n/a` and are corrected to `AND=n/a iOS=n/a DSK=yes
TV=n/a`. **C19**, a web updater fix, read `AND=n/a iOS=n/a DSK=yes` — and that stray `yes` was the
**TV** cell, so a shifted column was one step from recording a web-only fix as a desktop release. It
is corrected to `AND=n/a iOS=n/a DSK=n/a TV=yes`. All 88 feature rows now have eight columns, and
that is worth asserting on edit.
...
| shipped **DSK** | released on the **desktop** line, `0.1.23-alpha-z2`..`z6`, and still unreleased on the mobile line. Swept in Phase 6 Stage A, 2026-09-17: 54 rows moved here from `**branch**`. The state column is one cell for a row that spans four clients, so where a feature has shipped on one line and not another, this names which. Read it with the platform columns, which carry the per-platform truth |
| **branch** | built and unreleased. After the Stage A sweep this means *unreleased everywhere*, and only **D11**, **C9** (mobile) and **C19** (web) still carry it — desktop's release line never carried them, so `z2`..`z6` did not ship them |
...
| --- | --- |
| yes | present |
| **port** | planned for that platform, and **not built there yet** - a target, never a claim |
| **defer** | that platform is targeted, but this feature is deliberately deferred or unresolved there. The reason is in the row |
| **publish** | the platform participates as a *producer only* - it emits the data, and never renders or browses it |
...
`**defer**` and `**publish**` were added 2026-09-17, when Phase 6 set the Android/iOS targets and
Phase 10's TV target was decided early. They exist so a *planned* state can be recorded precisely
without touching what a platform actually has today. **`port`, `defer` and `publish` are all
targets**: a row only becomes `yes` in the same commit as the code that makes it true.
...
and distribution gate belongs to Phase 10 rather than Phase 7. A TV `port` target therefore does not
...
> carry **both** Social and Watch Together to Android and iOS, so S3 is no longer desktop-split. The
> platform split that remains is TV, which is excluded from Watch Together entirely.

**The TV column now carries a target** *(set 2026-09-17; it previously said "carries no target on
purpose")*. Phase 0 left it unset so that Phase 10 could cherry-pick the whole list, but that left
the ledger unable to answer what the television is *for*, which is a product question and not an
implementation detail. So the target is recorded here now and Phase 10 still owns building it, the
remaining per-feature judgement calls, and `nuviozweb/docs/Z-PORT-MATRIX.md`, which stays the better
answer for how a given feature has to change on its way to a television.

The TV target in one line: **playback in full, Social as a producer only, nothing interactive.**
Classic/Streamlined/Instant, ranking, failover, quality, the preferences that make sense on a TV,
automatic audio/subtitle selection, enhanced loading and next-episode all `**port**`. Downloads, Z
source setup, the full theme system, the Social tab, friends management, profile editing, the inbox
and Watch Together are all `**no**`. The presence and activity rows are `**publish**`: watching
something on the television publishes Watching Now and Recently Watched so friends on desktop and
mobile see it, and the set displays none of it, respecting the profile's existing Social and privacy
configuration rather than carrying a Social settings UI of its own.
...
> **Platform targets set 2026-09-17, when Phase 6 was rescoped to "Social + Watch Together to
> mobile".** Every row here now targets Android and iOS in full. Three things to read correctly:
>
> - **`port` on AND/iOS is a target, not a claim.** S1, S2 and S3 previously read `yes`/`yes*` on
>   mobile. That was wrong: the code is present but calls the official `api.nuvio.tv` client, which
>   hosts none of the `social_*` or `party_*` RPCs, so none of it has ever functioned on a handset.
>   They are corrected to targets here and become `yes` only in the commits that make them true.
> - **Watch Together is no longer desktop-only.** The `ships, **desktop**` v1 target on S3-S8 is
>   retired. The phase rests on an inspection: all 35 Watch Together files are in `commonMain` with
>   no desktop dependency, and party code reaches the player only through `PlayerEngineController`,
>   whose `samplePositionMs()` and `seekToExact()` were written for this. Mobile reuses that core
>   rather than growing a second implementation.
> - **iOS carries one deliberate deferral**, and it is not in this section: the built-in Z
>   source/addon setup flow (§5 **W-rows**). It does not affect anything here - iOS inherits a
>   configured source set through the existing per-profile addon sync.
>
> TV takes nothing from this section except S2, as a `publish`-only producer.

| # | Feature | State | **v1** | AND | iOS | DSK | TV |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **S1** | **Mutual friends and social profiles** — stable profile-UUID identity, unique handles, requests, removal, separate live/recent privacy controls, private Realtime invalidations and offline feed cache. **AND/iOS corrected to a target 2026-09-17:** mobile carries `SocialRepository`, `SocialModels`, `SocialScreen` and `HomeSocialSections`, but they call the *official* `SupabaseProvider` (`api.nuvio.tv`), which hosts none of the `social_*` RPCs, and the gate, preferences, notifications, presence session and presentation layer are absent — so this has never functioned on a handset and `yes` was a false claim. Phase 6 target. **TV `no`:** friends management and profile editing are not a television surface. | shipped **DSK** | ships | yes | **defer** | yes | **no** |
| **S2** | **Watching Now and Friends Recently Watched** — sanitized 20-second playback presence with a 90-second server TTL, permanent idempotent watched events, consecutive episode runs, Home rows and the Social tab. Provider imports never create activity. **AND/iOS corrected to a target 2026-09-17** for the same reason as S1 — the code is present but points at a backend with no `social_*` RPCs and has never published a real event from a handset. Phase 6 target. **TV `publish`:** watching on the television publishes Watching Now, playback/presence state where supported, and Recently Watched on completion, so friends on desktop and mobile see it; the set renders none of it and respects the profile's existing Social/privacy configuration rather than carrying a Social settings UI. Phase 10 target; see the open master-social-control question there. | shipped **DSK** | ships | yes | **defer** | yes | **publish** |
| **S3** | **Watch Together** — private eight-profile lobby, invite codes, independent source fingerprints, host/collaborative commands, deterministic host transfer, and a timing plane carried between clients over the party channel: positions paired with the instant they were read, a host-anchored clock, and play and seek scheduled at a shared instant. The host picks a source and then starts the party as two separate presses, so nobody leaves the lobby until the host says so. Desktop only so far; verified 2026-09-03 against a live two-profile party (both members reached `ready` on one source, `status=playing`). **Rescoped 2026-09-17: no longer desktop-only.** Phase 6 carries Watch Together to Android and iOS in full — creation, joining, Direct/Ask/Off, invites, lobby and readiness, independent per-client source matching and realization, synchronized playback, host vs collaborative control, episode handoff, in-player UI, connection/status UX, reconnect and recovery, host transfer and complete lifecycle. The durable core is reused, not reimplemented: all of it already lives in `commonMain` with no desktop dependency. **AND/iOS corrected to a target:** mobile's `features/watchparty` is a three-file stub from 2026-09-01/02 wired to the wrong backend and has never worked — Phase 6 deletes it and merges desktop's across. **TV `no`:** deliberately excluded, and not a Phase 10 target. | shipped **DSK** | ships | yes | **defer** | yes | **no** |

| **S4** | **Party episode handoff** *(desktop branch, unverified on hardware)* - the host advances the party to the next episode through the existing `party_change_content_v2`, one accepted content generation at a time; every client matches the equivalent source locally, publishes readiness and passes the existing barrier, then swaps episode **in place** with no route, controller or HWND teardown. Manual next, autoplay-next and the episode picker converge on one content-change path. The countdown is **host-only** - guests follow rather than choosing. The host's `PlaybackMode` decides how the *host* picks the source; guests never get an ordinary chooser. | shipped **DSK** | ships | yes | **defer** | yes | **no** |
| **S5** | **Social presentation of its own** *(desktop branch, unverified on hardware; redesigned 2026-09-15)* - Watching Now and Friends Recently Watched are two distinct components over shared low-level primitives rather than reuses of `TitlePresentationCard`, so a friend's history reads as ambient social activity and stays visibly subordinate to the viewer's own Continue Watching. Friends' activity is **grouped by title** (one row per title, however many runs or friends) as a 2:3 mini-poster beside an avatar stack, the title and an episode span, with no card surface at rest; Social buckets it into Today / Yesterday / This week / Earlier and pages automatically. Watching Now is identity-first (avatar with a playing/paused dot, progress on the artwork) and never merges two people's sessions by matching content. Its tonal action follows the effective join policy and the outgoing request - Join, Ask to join, Requested, Joining…, In your party, or nothing. | shipped **DSK** | ships | yes | **defer** | yes | **no** |
| **S6** | **In-player Watch Together panel** *(desktop branch, unverified on hardware)* - the header button only opens a panel; it never creates a party. The panel has a state for every situation, including not being in one: unshareable source, idle with **Start a party**, starting, start failed with Try again, connecting, active (host or guest), in a party elsewhere, and ended. Active shows one connection chip (Live / Reconnecting… / Delayed / Offline), people with plain-language status, inline invites with a sent state, host settings written as switches, **Who can join** (Direct / Ask / Off, optimistic with rollback - moved off the player header), a Leave that says who becomes host, and an End that asks inline. | shipped **DSK** | ships | yes | **defer** | yes | **no** |
| **S7** | **Ask-to-join lifecycle** *(desktop branch, unverified on hardware)* - an outgoing request is a process-owned store that survives tab changes, the lobby and the player: pending with an expiry ring and Cancel, declined / expired / friend stopped watching with Ask again, and accepted as a 3s countdown while browsing or an explicit Join / Not now inside your own player, handing off through a lobby hero and loading line that name the person. The request is bound to the profile and token that sent it and is cancelled as that profile at every identity boundary (profile switch, sign-out, Social off, wipe, capability loss). Backed by `social_join_request_status`, `social_cancel_join_request` and a broadcast trigger for immediate delivery to both sides (deployed to `pzbpghmmordvzcfbayoh`). | shipped **DSK** | ships | yes | **defer** | yes | **no** |
| **S8** | **Party status pill** *(desktop branch, unverified on hardware)* - one debounced line over the video about what the party is waiting on, with at most one action, projected by a pure priority table: a guest's own source or episode handoff (Choose source), a too-short version, the host choosing, the start gate (Start anyway), a stall hold seen from both sides (Don't wait), host buffering, catching up, an incoming request (Let in / Decline), an outgoing request (Cancel), reconnecting, offline, and **Paused by**. Pause is a condition, not a toast, and a host stall hold travels on the timeline tick (`PartyTick.hold`) so it is never read as a person pausing. Compacts rather than hiding when the chrome fades. | shipped **DSK** | ships | yes | **defer** | yes | **no** |
| **S9** | **Timeline-safe party source authority** *(both branches, hardware-verified 2026-09-20 for the seek/readiness half)* - the invariant is **different URLs are fine, different timelines are not**. The host defines the party timeline, so a host whose chain lands on the same authoritative release (renewed URL, re-mint, same torrent file) retries locally and the party never moves, while a host that genuinely leaves its release advances `sourceGeneration` and takes everyone with it. A guest's failure never changes the authoritative source: it may silently fall back only to a source proven timeline-compatible, and otherwise stays in the party, withholds readiness and is offered the source UI rather than reporting a ready it cannot honour. Duration agreement is a contradiction check, never a proof. Seek-caused buffering is resolved by **positive readiness** - everyone stays paused, lands on target, republishes post-seek readiness, and the existing play barrier resumes the party together - with the exit named in the log as `all-ready`, `dont-wait`, `ceiling` or `degraded`. The startup watchdog no longer freezes for a party hold until the source has proven viability through real media progress, so a dead source cannot become immortal behind the start gate; a parsed duration, a reachable URL and a party-commanded seek are explicitly not proof. **Manual host selection is authority:** a host picking a different release from the sources panel advances the party even at the `EquivalentMedia` tier, while re-picking the party's own release, any automatic path, and any guest's pick all keep the full duplicate test. Hardware run 2026-09-20 (Android `z1.35` + desktop `z6.56`, both seek directions): every barrier resume was `all-ready`, the 12 s ceiling never fired, worst wait 8.5 s. **Real source-failure paths are unverified** - they cannot be forced reliably and are trial-by-fire from normal usage. | **branch** (AND + DSK) | ships | yes | **defer** | yes | **no** |
| **S10** | **Away, and “Pause for away users”** *(both branches, unverified on hardware)* - a member who backgrounds the app, locks the screen or dismisses a picture-in-picture window into the background is **Away**: still in the party, deliberately not watching. It is explicitly not disconnected, buffering, leaving, a source failure or evidence about the host. Before this, `ON_STOP` paused the engine and all the party could see was a member that had stopped reporting `playing`, so the stall guard read somebody pressing Home as a stalled stream, held the party and abandoned them at the hold ceiling - and `ON_START` then resumed them at the stale frame for the drift tracker to drag forward. **Picture-in-picture is not Away:** the lifecycle arrives as a snapshot with a monotonic sequence rather than as events, so the four callbacks a PiP transition delivers in an unguaranteed order cannot flap Watching↔Away, and a late callback is dropped whole. **Returning never resumes at the stale local position:** the party's content and source generations decide between a catch-up and a realization, and an unchanged generation means the open stream is still the party's - no realizer reset, no route disposal, no readiness downgrade, no re-resolution. The catch-up is the existing `partySeekPlan` and barrier park, taken at a known moment instead of waited for. **“Pause for away users” is a host switch separate from “Pause when someone buffers”**, defaulting off, with its own reactor and its own held list so neither hold can release the other; the status pill says *Waiting for … to return*, never *Buffering*. A host's own absence pauses the party whatever the switch says, because the host is the clock, and it is issued as a real command so it is ordered and attributable. **Host authority is untouched** - Away writes no member row and transfer stays `party_transfer_stale_host`. Member rows are `Watching` / `Away` / `Buffering` / `Reconnecting` / `Offline`, with transport outranking Away. On the wire: two additive optional fields, no protocol bump, and the shipped desktop release decodes every message unchanged. Desktop produces no local Away by design - window focus is not somebody leaving - but consumes every part of it. | **branch** (AND + DSK) | ships | yes | **defer** | yes | **no** |
...
> **Phase 6 closeout, 2026-09-21.** The `port` targets above are now `AND yes`. The claim rests on the device, not the compile: Android↔desktop parties in both directions (seek barrier, timeline-safe source authority, Away and its return, all on the S25), and the phone Social tab, lobby and in-player surfaces accepted on the S25. The iOS column reads `**defer**`: built from the same `commonMain` code and compiling in CI, with physical verification and two adapter items owned by Phase 8 - `iosApp/Player/NowPlayingController.swift` moves the player directly instead of through `PlayerExternalTransport`, so a lock-screen pause never becomes a party command; and `PlayerEngine.ios.kt` reports no `engineReadiness`, so starvation uses the buffered-ahead fallback. **Not walked, carried as QA debt (§12):** an Android↔Android party, doze after 10+ minutes, an incoming call and a Wi-Fi↔cellular handover mid-party, and the natural source-failure paths of S9.
...
| **D16** | **iOS background transfers and queue ownership** - downloads run as background `URLSession` tasks that survive screen lock and relaunch. The repository owns the queue, a live task is the only proof a download is running, and relaunch adopts the tasks that really exist instead of reordering. On iOS there is no concurrency cap: while the app is open the repository resolves and submits a window of up to 12 items from the one global queue, created in queue order with a priority hint, and the system runs them while locked (a task created from the background is discretionary and rate-limited, which is why `.45`'s one-at-a-time background chaining stalled until unlock; it survives only as best effort beyond the window). A force-quit requeues the window in place without charging attempts. `.44` ran two schedulers against one queue and started #3/#4 ahead of #1/#2; `.45` replaced it. Plus a Live Activity with one stable session and a queue summary in the foreground, which turns queue-level ("Nuvio Z Downloads / Downloading in background", no episode, percentage or counts) while the app is suspended, since nothing it could show there stays true. `.47` also stops the batch review from approving sources the debrid service has not cached (they could never resolve) and sends them to the manual source picker. `.48` matches native tasks by `taskIdentifier`, drops finished tasks before any inventory or start, and sizes a resumed 206 from `Content-Range`: `.46` had deleted a complete file that ended on a 206, then re-attached to the finished task as a fake live 100%. **Physically verified:** locked-screen continuation (`.46`/`.47`). **CI only:** the `.48` fixes. Concurrency, attention and source-policy design moved to Phase 9 (Downloads Redesign). | debug (`.48`) | ships | n/a | yes | n/a | n/a |
...
awaits a decision (Phase 8 audit, Batch 4 UI review). **Moved to Phase 9 §J** (Library ↔ Downloads). The shared change reaches
desktop only through `NuvioZDesktop` branch `claude/phase-8-shared-convergence`, which is not merged to `Dev` until that decision is made.
...
> **TV target, set 2026-09-17.** Playback is the part of Nuvio Z the television gets essentially in
> full: Classic, Streamlined and Instant, source ranking, failover, quality selection, the playback
> preferences that make sense on a TV, automatic audio/subtitle selection, the enhanced loading
> experience and next-episode/autoplay. The per-row TV cells below already carry `port` with their
> porting notes; Phase 10 settles the remaining per-feature judgement calls against this target
> rather than deciding the target itself. `**no**` on **P4** is not an exception to it - that row is
> a *download* entry point, and downloads are not a TV target.
...
**Phase 10 web drift:** NuvioZWeb reimplements the Kotlin playback decisions rather than sharing
...
| # | Feature | State | **v1** | AND | iOS | DSK | TV |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **S1** | **Quality options derived from the catalogue** - instead of a fixed tier list filtered down, the real candidates are bucketed by resolution and each bucket banded. An empty bucket produces no row, so a title with no 4K release simply has no 4K option, and each row quotes the connection speed the file it would actually open needs. | shipped **DSK** | ships | yes | yes | yes | modules **landed** |
| **S2** | **Absolute quality bands** - Max / High / Mid / Low with fixed Mb/s boundaries (4K 10/25/50, 1080p 3/8/16), replacing geometric thirds of each title's own spread where "4K High" meant an 88 GB remux on one title and a 14 GB WEB-DL on the next. Fewer than two occupied bands collapse to one unlabelled row: a lone row reading "1080p Mid" is a comparison with nothing to compare against. Sizeless sources join the cheapest band rather than forming a phantom Low row. | shipped **DSK** | ships | yes | yes | yes | modules **landed** |
| **S3** | **Quality selector grid** - one card per resolution with the bands stacked inside as the tap targets, in a responsive grid: a centred panel at 768 dp and above, a modal bottom sheet below. The body renders **one of three states and never a blend** - a skeleton while the figures are still moving, the final grid, or the no-match text. Dismissing returns to details; only "Choose source manually" reaches the source list. | shipped **DSK** | ships | yes | yes | yes | **port** |
| **S4** | **Preferences dialog on the sheet** - a dialog, not a navigation, because Settings is a different back stack and going there would cost the user the episode. Rows cycle rather than opening pickers, writes go through the real repository setters, and the grid behind rebuilds itself. | shipped **DSK** | ships | yes | yes | yes | **port** |
| **S5** | **Codec, HDR, audio-format and language preferences applied to playback** - these existed for downloads only and were dead on the playback path, so `REQUIRE_HDR` meant nothing. They now decide what auto-pick opens. `REQUIRE_*` **demotes by -100 rather than excluding**, so the source stays in the failure chain; downloads still exclude. | shipped **DSK** | ships | yes | yes | yes | **port** |
| **S6** | **One release-name parser** - the app had **two that disagreed about the same file**: the badge row read HDR10+, Dolby Vision and Atmos correctly while the picker read them as SDR. Both now delegate to one import-free parser, and four defects died with it - a regex that backtracked so every HDR10+ release was labelled HDR10, `hdr10plus` matching nothing at all (so such a release read as SDR and was ranked *below* a plain HDR one by the very preference asking for HDR), `dovi` unrecognised, and substring matching so `cam` hit inside *Camelot*. | shipped **DSK** | ships | yes | yes | yes | **landed** |
| **S7** | **Language vocabulary and the language gate** - the token list knew **seven** languages, so Hindi, Italian and Russian releases declared nothing and were indistinguishable from an untagged English one; `MULTi` and `DUAL` were unrecognised and are not languages; flag emoji were unhandled, which is how Torrentio, Comet and MediaFusion all label audio. Now ~120 ISO aliases and ~72 names. The gate **partitions, never filters**: deleting unwatchable candidates empties the chain on a title released only for another market. | shipped **DSK** | ships | yes | yes | yes | **landed** |
| **S8** | **Plausibility ceiling, fake-8K demotion and AI upscale / CAM ranking** - an 85 GB "1080p" season pack used to head every row because size sorts descending. A per-resolution plausibility ceiling now stops an implausible size heading a row, setting its bandwidth or setting its displayed size. Separately, nominal 8K AI upscales fold into 4K tier on displays below 8K (< 4320p), AI-upscaled releases receive an `AI_UPSCALE_PENALTY` (-8) and restrained danger chip, and CAM/TS releases receive a `CAM_TS_PENALTY` (-20, tier -1) with restrained provenance styling. Proper 4K releases comfortably beat nominal 8K AI upscales. | shipped **DSK** | ships | yes | yes | yes | **landed** |
| **S9** | **Failure chain and dead ends** - a family of fixes so no path leaves the user on a covered blank screen: the chain is capped to its budget, the P2P-decline and uncached and early-return paths all report something, both unbounded waits are bounded, a dead debrid link can no longer loop forever inside the player, backing out of a not-yet-started player no longer relaunches it, and backing out of Streamlined lands on details in **one** press rather than the source list. **Phase 2 extended this.** Desktop's `onFatalPlaybackError`/`onPlaybackStarted` had been dropped by the `0.1.22-alpha` sync, which left the startup watchdog unarmed, the post-playback failover chain unable to advance, and `consumeFailoverRetry()` permanently false - restored. Every position derived from a duration now goes through pure `PlaybackPosition`, which clamps and refuses implausible durations by name rather than seeking into them. `AddonStreamGroup.error` is no longer discarded: it prints in the list and becomes the failure reason when the caller has none. All 13 ways into the source list carry a named path and log it, and `hasSilentUncover` makes "the list appeared and nothing said why" a failing test rather than a convention. | shipped **DSK** | ships | yes | yes | yes | **port** |
| **S10** | **Unknown is not cached** - Instant auto-played a not-cached-yet placeholder because the cached flag was *null* rather than false. Auto-pick now requires positive evidence of a cached copy, and uncached candidates stay out of the fallbacks list. Scoped to debrid-backed candidates only: a plain HTTP link legitimately has no cache state. | shipped **DSK** | ships | yes | yes | yes | **landed** |
| **S11** | **Debrid stream preferences without a built-in cloud account** - a user whose debrid runs *inside* the addon (AIOStreams and anything like it) had the **entire Debrid settings page doing nothing**: filter, sort, cap and name templates were all gated on owning your own API key. New persisted scope - resolver-only / debrid / **all addon streams**, defaulting to all - with a Stream preferences section that is always enabled, a hint when no account is connected, and AIO metadata read as a fallback for resolution, codec, HDR, audio, languages, size and sub-addon name. | shipped **DSK** | ships | yes | yes | yes | **port**, after Phase E |
| **S12** | **"No streams found" over a full catalogue** - the list auto-filtered to the addon that last served a show, but a group exists for every addon that was *asked*; filter to one that answered nothing and the empty state drew over everything the others found. | shipped **DSK** | ships | yes | yes | yes | **no** - upstream bug, send it to vanilla |
| **S13** | **Plugin metadata survives ingestion** - scraper results had quality, size, seeders, peers, provider and language joined into one display string, dropping seeders and peers entirely, which the facts extractor then regexed back apart. Now a typed record. | shipped **DSK** | ships | yes | yes | yes | **no** - no JS plugin runtime on TV |
| **S14** | **One language decision, honoured by both halves** *(desktop branch)* - the picker and the player disagreed about what a language preference meant. `preferredAudioLanguage` ships as the sentinel `device`, and the source picker discarded every sentinel (`device`, `default`, `original`) one call before ranking read it, so **`LanguageStrictness.REQUIRE` - the shipped default - did nothing at all** for any profile that had never opened the language dialog. One pure resolver now settles the sentinels for ranking exactly as the player settles them for track selection: `device` to the OS locale, `original` to the title's own language (fetched for movies only when that preference is set), and a sentinel naming no language to no opinion rather than to a guess. Subtitle language joins the ranking as a **bonus that promotes and never demotes**, because source names are thin on subtitle metadata. The default strictness drops `REQUIRE` -> `PREFER` in the same change, and a one-shot migration writes the resolved device code over the sentinel, so nobody is silently promoted from an inert requirement to a live one. All four language rows now sit in one **Language** section on the Playback page, ungreyed in every mode, and the wizard asks for them once. | shipped **DSK** | ships | yes | yes | yes | **port** |
| **S15** | **The language slot tells the truth, or says nothing** *(desktop branch)* - the loading band printed `SourceFacts.languages` verbatim. That field is only ever a *positive* release-name claim and English is the unmarked case, so an English release showed **nothing** while a tagged release showed its tag - the absence read as "this source has no language", which is the one thing a release name can never say. Worse, `VOSTFR` / `LEGENDADO` / `ENG.SUBS` / `MultiSubs` were all counted as *audio*, and the subtitle list fell back to the parsed audio languages. `releaseLanguageEvidenceIn` now splits audio evidence from subtitle evidence and adds `DUBBED` / `HC` markers; `SourceLanguageInference` resolves in a fixed order - structured field, release name, then the title's own original language, **read synchronously from already-loaded meta so the band never changes under the reader**, and skipped outright when `DUBBED` or `MULTi` contradicts it - and otherwise says unknown. Subtitles never fall back to the title's language. A production country is never read as a language here, because this value is *displayed*. | shipped **DSK** | ships | yes | yes | yes | **port** |
| **S16** | **Prefer built-in subtitles** *(desktop branch, unverified on hardware)* - `playback_prefer_embedded_subtitles`, off by default, synced, in the Source preferences section. Active **only** for Streamlined/Instant automatic picks: Classic, every manual pick and every download return null, and the player additionally gates on the launch having been auto-picked. Two halves that never fetch anything. *Before* open, `SourceRanking.embeddedSubtitleScore` gives +2 for a release-name subtitle claim in the primary subtitle language and +1 for `MultiSubs`, inside `mediaScore` only - it never touches the resolution or language tier, and a hard-subbed release scores 0. *After* open, `verifyEmbeddedSubtitles` reads mpv's `track-list` (all three native bridges now emit `external` and `default`): a non-forced container track in the target language beats a sidecar or addon track in that language, `default` breaks ties, and **any** other outcome logs once and falls through to the unchanged auto-selection. No source retry; forced-only mode never enters the path. ⚠ **The quality panel deliberately says nothing about subtitles** - a chip was built and removed the same day on hardware evidence, because its *absence* read as a denial the release name had not made. | shipped **DSK** | ships | yes | yes | yes | **port** |
...
| # | Feature | State | **v1** | AND | iOS | DSK | TV |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **W1** | **The ten-step wizard** *(revision 8, desktop branch; mobile is still on revision 6's eight steps)* - Welcome, playback mode, **playback setup**, **language**, sources, **social opt-in**, **social identity**, look, theme, done. Three of the ten are conditional: playback setup only for a mode that has something to ask (Classic has nothing), sources only without an enabled addon, social identity only with social on and no handle yet. **Language is asked in all three mode branches**, Classic included: everything the playback-setup step asks feeds the automatic source picker, which Classic has none of, while the language answer also drives the player's own track selection, which runs in every mode. Appearance is down from four steps and fourteen controls to two and four. Two opaque regions and **nothing is ever drawn behind text**: a full-bleed specimen band showing what this step changes, an opaque panel of at most four controls, a hairline between. Nothing exceeds four controls so **no panel scrolls on a phone**. An optional step is *dropped*, not shown-and-skipped; and a step that leaves the plan while the user is standing on it - all three can - heals forward rather than stranding them. Choices apply the instant they are tapped, through the real settings setters. | shipped **DSK** | ships | yes | yes | yes | **no** |
| **W2** | **Welcome is a real screenshot** - the app's actual home composables at the app's real metrics with a seeded list state so the hero parallax is the app's own, under a blur, pointer-blocked so it cannot scroll under your thumb. | shipped **DSK** | ships | yes | yes | yes | **no** |
| **W3** | **The playback-mode storyboard** - an animated loop per mode from tapping a title to playing it. Classic stops on a wall of releases with a finger walking every row; Streamlined asks one short question and settles a release **with no pointer anywhere**; Instant goes straight to play. The sequences are data in an import-free file, so "Streamlined picks the release itself" is a claim a test can hold. | shipped **DSK** | ships | yes | yes | yes | **no** |
| **W4** | **Show-once-by-revision** - an integer, profile-scoped and synced, currently at **revision 8** on desktop and 6 on mobile (8 added the language step). A boolean could never re-ask, and keying on the app version would re-show the whole wizard every release. Finishing used to also mark the old mode selector seen; `playback_mode_selector_seen` is deleted - nothing had read it since the wizard replaced that selector - and the key survives in `syncKeys` only, as a tombstone that lets an older client's payload clear the orphaned local value. Settings, About, **Run setup again** re-runs it dismissibly, over the app rather than gating it. | shipped **DSK** | ships | yes | yes | yes | **no** |
| **W5** | **Fetched sample artwork** - public metadata-hub URLs, never bundled, because poster art is copyright and both repos are public. The wizard must be fully usable with no network. | shipped **DSK** | ships | yes | yes | yes | **no** |
| **W6** | **Wizard-adjacent fixes that reached the real app** - a root that consumed no pointer input, so taps missing a control went through to the page underneath (**the second time that defect shipped**; now a rule in `AGENTS.md`); "Group sections into tabs" doing nothing anywhere, because a tab group needs more than one member and normalisation seeded every section with none; and details-section tabs below the interactive minimum tap target, with the outgoing crossfade half hit-testable for 200 ms. | shipped **DSK** | ships | yes | yes | yes | **no** |
| **W8** | **Social opt-in as a real feature preference** *(desktop branch)* - the wizard's social step gates the whole social product layer, not just handle setup: with it off the Social tab, the home activity rows, both Watch Together entry points, presence, the realtime channel, activity publishing and the notification prompts are gone, and the runtime never starts. The one deliberate exception is the master control in Settings → Social. Turning it off **deletes nothing** - handle, friends and activity all survive, and re-enabling brings them back. Profile-scoped and synced through its own payload, stored nullable so an established social user is never hidden by a new boolean defaulting false. | shipped **DSK** | ships | yes | yes | yes | **no** |
| **W9** | **Ordered social shutdown** *(desktop branch)* - turning social off while in a Watch Together party departs it through the Phase 4 coordinator first, waits for the phase to settle, then clears the profile, then closes presence and realtime - and only then writes the preference, so the surfaces cannot vanish out from under an in-flight departure. Lifecycle only; no party authority, sync or matching logic is touched. | shipped **DSK** | ships | yes | yes | yes | **no** |
| **W10** | **Z source setup, built in** *(desktop, shipped in `0.1.23-alpha-z4`; row added 2026-09-17 - it shipped without one)* - the wizard's Sources step installs a working stream source in place rather than sending the user off to configure a Stremio addon by hand: a recommended AIOStreams + TorBox card, the Z template fetched from the templates branch at setup time, and the key link, all through the ordinary `AddonRepository.addAddon` installer, so nothing about the addon model is special-cased. **iOS `defer`:** deliberately *not* targeted, to minimise App Store review risk around a conspicuous in-app third-party addon/source installation surface. This is deferred and unresolved, not solved - there is no workaround here and none is wanted, and nothing is designed to hide functionality from review. It costs iOS nothing architecturally: addons are per-profile and sync through the official backend (`AddonRepository.pullFromServer` / `pushToServer`), so an iOS user signed into the same account inherits a source set configured on Android or desktop, and playback, Social and Watch Together are unaffected. ⚠ **Not implemented yet (found at the Phase 6 closeout, 2026-09-21):** `setupWizardSteps` keeps Sources unconditionally on every platform, so an iOS build shows this step today. Adapting it out on iOS is a Phase 8 item. **AND `yes` 2026-09-21:** the step is shared `commonMain` code and Android runs the same wizard. | shipped | ships | yes | **defer** | yes | **no** |
| **W7** | **Render harness** - every specimen at two widths in every palette, with and without AMOLED, plus every storyboard frame, plus **three full walks through the conditional flow** (Streamlined, Instant, Classic-with-social-off) at four window sizes, written as PNGs and uploaded by CI on every push. It has already earned itself twice: it caught a playback step whose chips truncated four distinct dynamic-range options into two readable words. | shipped **DSK** | ships | n/a | n/a | yes | n/a |
...
| **The setup wizard** | Revision 6 walked end to end on a real desktop build (maintainer, 2026-09-10). | ⚠ Revisions 7 and 8 have **not** been on hardware at all - 8 adds the language step, which needs walking in all three mode branches - and `Settings → Run setup again` has had no physical result since the pointer-consumption fix. |
| **The settings reorganisation** | Playback, Advanced and Subtitles walked on a real screen. | - |
| **The social tab** | Handle setup, friends and recent activity used against the Z backend on a real build (maintainer, 2026-09-10). | The new Settings → Social page is unverified. |
| **The web port on a television** | It ran on a TV. | Buggy. Phase 10 owns the bugs; the claim being settled here is only that it runs at all. |
...
| **iOS has been run - on SideStore, not TestFlight.** | *Revised 2026-09-24, Phase 8 closeout.* Unsigned Debug IPAs `.41`-`.48` from the SideStore debug feed have run on a physical iPhone. What that proved is recorded in `ROADMAP.md` Phase 8 and `PHASE-8-IOS-BRINGUP-AUDIT.md`: locked-screen background downloads (submitted window), queue order, foreground download and offline playback, and Files-visible diagnostics. What is **still open**: most `.43`-`.48` checklist rows were never reported back (including every `.48` fix); signing, an Apple Developer account and a TestFlight build do not exist. **No stable mobile release follows Phase 8**; see Phase 9's gate. |
| **The whole consolidated desktop branch (2026-09-16).** | `nuviozdesktop` `claude/desktop-consolidation` merges two lines that had never been built together. Automated gates are green - `desktopTest` 2,050 tests / 0 failures, all pure suites, backend pgTAP - and **nothing on it has been on hardware**. A short checklist covering only the changed seams is at `HANDOFF-desktop-consolidation.md`. Broad physical QA stays deferred; this is QA debt, not open development. |
| **The friend's playback fixes, across the merge.** | Premature EOF and the AIOStreams **Wrong IP** failure were reproducible on a friend's machine and **passed on his dedicated test build**. That build was `01524bb1`, which predates the built-in-subtitle work and sits on the far side of the consolidation merge from the preferences pass. The fixes are verified *as they were then*; re-confirmation on the consolidated MSI is owed. |
| **Prefer built-in subtitles (S16).** | Never exercised. Needs a known embedded-subtitle MKV for the confirm path and a source with no built-in track for the fall-through path. The post-open `track-list` verification is the part with no hardware evidence at all. |
| **iOS Social and Watch Together, physically.** | *Opened 2026-09-17, for Phase 6.* Phase 6 implements Social and Watch Together on iOS rather than deferring them to Phase 8, because the architecture is shared and leaving a whole stack unwritten would only move the work. But **there is still no Apple Developer account**, so none of it can be run on a device. What Phase 6 can honestly claim for iOS is: it builds, the shared implementation is the *same* implementation Android uses, the platform adapters compile, the iOS TODOs are adapter details rather than architectural placeholders, and the shared/pure tests are green. Everything else is debt, and Phase 6 carries a **precise physical-verification matrix** - the Android cross-platform matrix, restated for iOS - written to be *run* when hardware arrives rather than invented then. **Nothing in it may be marked verified, and no iOS row here may become `yes` on the strength of a compile.** Phase 8 owns discharging it. **2026-09-24: Phase 8 did not discharge it.** Phase 8 went to downloads; no iOS party or Social pass was reported. The matrix, the `NowPlayingController` lock-screen transport bypass (Android has `PlayerExternalTransport`, iOS Swift calls mpv directly) and the missing iOS `engineReadiness` now sit on the **mobile stable release gate** (`ROADMAP.md`, Phase 9 §L). |
| **Phase 6 lifecycle matrix, on Android.** | *Opened 2026-09-21 at the Phase 6 closeout, which the maintainer closed without it.* Never run: a party between two Android phones (S20+ and S25+), a phone left in doze for 10+ minutes mid-party, an incoming call, and a Wi-Fi↔cellular handover. Android↔desktop in both directions and the screen-lock Away return **have** been run. Non-blocking QA debt; run it opportunistically, and preserve logs when something goes wrong. |
| **Watch Together on desktop, formally.** | *Corrected 2026-09-17.* `PHASE-4-TWO-CLIENT-VERIFICATION.md` marks all 21 scenarios `NOT RUN`, which now **understates** reality: recent builds have had real **three-user** party use, host transfer included, and it has held up - which is why desktop is treated as a trusted reference peer for Android↔desktop testing in Phase 6. What is genuinely open is that no formal matrix has been *walked*, so the untested edges are unknown rather than known-good. Phase 6 Stage A records what has actually been observed beside the still-unwalked table instead of leaving the blanket `NOT RUN` to be read as "never worked". |
| **The Playback settings page, visually.** | ⚠ **It has no render harness** - only setup and social do - so the new Language section and the metered-cap row have been compiled and never drawn. Everything else on that page that moved in the preferences pass is in the same position. |
