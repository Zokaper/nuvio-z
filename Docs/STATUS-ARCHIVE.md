# Nuvio Z status archive (nuvio-z)

**History only. Nothing here is a live claim.** This is the part of `STATUS.md` that had stopped
being a handoff and become a record: every entry dated before 2026-08-24, plus the standing
`Current Snapshot`, `Completed`, `Verification` and `Work Log` sections, split out on 2026-09-04 in
Phase 0 of `ROADMAP.md`. Sections are unedited and in their original order.

**`STATUS.md` is the live handoff.** Read that first. Come here for why something was done, for a
device script an entry defined, or to confirm that a problem you are looking at has been seen
before - that last one is most of the value.

⚠ **`Current Snapshot` below is stale** and was already stale when it was archived: it names `main`
and `Dev` as the working branches, which carry no product code. The state table at the top of
`STATUS.md` supersedes it.
