...
Latest pass on `claude/phase-2-desktop-handoff` remains **IN PROGRESS pending watched MSI verification**. Desktop now collapses and hides the fullscreen Swing interop wrapper synchronously at exit, waits for the previous Compose destination's real T2 draw before starting native teardown, and applies a short Compose pop fade-through only after native airspace is gone. The watched attempt 2 was proven to be mixed `DoVi HDR10`, not SDR; canonical failover order was intact and now has explicit regression coverage. Shared parsing already retained both capabilities internally and matched both ranking predicates, but the shared UI formatter discarded HDR whenever DV was present; it now emits `HDR10/DV` (or `HDR/DV`) with mixed-syntax tests. Mobile/shared verification passed: six pure groups (409/409 at the shared commit) and `:composeApp:testAndroidHostTest` (1,398/1,398). Desktop details and the fresh diagnostic MSI are recorded in `nuviozdesktop/STATUS.md`.
