...
| **Next** | Release `0.4.0-beta` — the first shared version number across both apps (see "Versioning" below). Then: compile the iOS Swift buffer fix on a macOS host, smoke-test the playback modes on a device and on the installed Windows app, and cover a real `NetworkStatusRepository` offline/online transition. |
