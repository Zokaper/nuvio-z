...
Watched desktop MSI update: mixed HDR/DV presentation is confirmed working (`HDR10/DV`), source/play to loading and the existing 1dp startup gate remain clean, canonical failover order remains correct, and one Escape exits without returning to stale loading. Phase 2F is **STILL OPEN**: player to details remains a jump cut with a short black interval; loading to player may expose a dark/light native placeholder or controls before real video; and consistent fade choreography is still pending. The next agent must diagnose those desktop visuals from the current baseline without regressing the shared HDR/DV fix or established playback/failover behavior. Full detail is in `nuviozdesktop/STATUS.md`.

Latest pass on `claude/phase-2-desktop-handoff` remains **IN PROGRESS pending final visual handoff work**. Desktop now collapses and hides the fullscreen Swing interop wrapper synchronously at exit, waits for the previous Compose destination's real T2 draw before starting native teardown, and applies a short Compose pop fade-through only after native airspace is gone. The watched attempt 2 was proven to be mixed `DoVi HDR10`, not SDR; canonical failover order was intact and now has explicit regression coverage. Shared parsing already retained both capabilities internally and matched both ranking predicates, but the shared UI formatter discarded HDR whenever DV was present; it now emits `HDR10/DV` (or `HDR/DV`) with mixed-syntax tests. Mobile/shared verification passed: six pure groups (409/409 at the shared commit) and `:composeApp:testAndroidHostTest` (1,398/1,398). Desktop details and the fresh diagnostic MSI are recorded in `nuviozdesktop/STATUS.md`.
...
- **Current Verified State & Handoff (Phase 2F IN PROGRESS - NOT COMPLETE):**
  - *Startup Handoff:* 1dp `SwingPanel`/native-airspace gate verified working; source → loading is visually smooth with heavyweight native player concealed underneath until first frame. Do not regress or redesign this.
  - *Native Airspace:* AWT/Win32 HWND occludes Compose; fullscreen hidden `SwingPanel` causes white screen via Skia hole-punching. 1dp parking until first frame is the proven working architecture.
  - *Attempt 1 Failure Diagnostic:* Genuine stalled remote CDN stream on TorBox; watchdog functioned correctly at 35s; Attempt 2 succeeded under identical gate in 13.3s.
  - *Player Exit:* Requires only one Escape, skipping transient `StreamRoute`. Fatal error and manual source selection retention preserved. **REMAINING DEFECT:** Player → previous screen still shows a visible WHITE flash during exit. Not visually complete.
  - *Open Work for Next Agent:*
    1. Transition Polish: eliminate white flash on exit; provide controlled fade-out/fade-through presentation across all handoffs respecting HWND airspace without arbitrary sleeps.
    2. Candidate Ranking Investigation: determine why failover selected 4K SDR when 4K HDR was available (verify candidate list consistency).
    3. Mixed HDR/DV Classification: investigate whether streams with both HDR10 and DV lose HDR internally or only display "DV" in UI; ensure both capabilities retained, matched, and labeled (e.g. `HDR10/DV`).
- **Verification & Artifacts:** 2026-09-07 handoff rerun of focused `:composeApp:desktopTest` passed 36 tests: `PlayerExitNavigationTest` (6), `NativePlayerAirspaceGateTest` (4), `NativePlayerControllerTeardownTest` (23), and `NetworkQualityPlatformDesktopTest` (3). Earlier pure suites passed (459 tests); `PlayerExitOrderingTest` passed (4 tests). Packaged Windows release MSI with debug tools: `composeApp/build/compose/release-msis/Nuvio-Z-Windows-x64-0.1.22-alpha-z1.msi` (258,299,679 bytes).
