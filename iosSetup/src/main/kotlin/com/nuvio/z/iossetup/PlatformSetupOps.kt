...
class WindowsSetupOps(diagnostics: Diagnostics) : ProcessPlatformOps(diagnostics) {
    ...
    override fun checkComputer(): ComputerCheck {
        ...
        val service = run("powershell.exe", "-NoProfile", "-NonInteractive", "-Command", "(Get-Service -Name 'Apple Mobile Device Service' -ErrorAction SilentlyContinue).Status")
        val registry = run("powershell.exe", "-NoProfile", "-NonInteractive", "-Command", "if ((Test-Path 'HKLM:\\SOFTWARE\\Apple Inc.\\Apple Mobile Device Support') -or (Test-Path 'HKLM:\\SOFTWARE\\WOW6432Node\\Apple Inc.\\Apple Mobile Device Support') -or (Get-AppxPackage AppleInc.iTunes,AppleInc.AppleDevices -ErrorAction SilentlyContinue)) { exit 0 } else { exit 1 }")
        ...
        return ComputerCheck(
            ...
            CheckResult("Apple Mobile Device Service", if (service.details.contains("Running", true)) CheckState.PASS else CheckState.ACTION, if (service.details.contains("Running", true)) "Running" else "Install or restart Apple device support."),
